...
- **MiniMax Leaks Its `<think>` Text.** Reasoning arrives as literal `<think>` text inside every reply, so grep for it
  and prefer `deepseek-v4-flash-gonka` when the leak matters.
